import java.io.*;
import java.nio.file.*;
import java.security.*;
import java.security.spec.PKCS8EncodedKeySpec;
import java.util.*;
import java.lang.reflect.Method;

/**
 * Decryptor v3 - uses FR's EncryptionToolBox.RSA directly via reflection.
 */
public class ResourceDecryptor3 {

    private static final String PRIVATE_KEY_B64 =
        "";

    public static void main(String[] args) throws Exception {
        String resourcesDir = "C:\\workspace\\res";
        String outputDir  = "C:\\workspace\\res_decrypted_v3";

        if (args.length >= 1) resourcesDir = args[0];
        if (args.length >= 2) outputDir = args[1];

        System.out.println("=== Resource Decryptor v3 (using FR EncryptionToolBox) ===");
        System.out.println("Input:  " + resourcesDir);
        System.out.println("Output: " + outputDir);

        // Load FR EncryptionToolBox.RSA by adding fine-cbb-11.0.jar
        File frJar = new File("D:\\FineReport_11.0\\webapps\\webroot\\WEB-INF\\lib\\fine-cbb-11.0.jar");
        java.net.URLClassLoader frCL = new java.net.URLClassLoader(
            new java.net.URL[]{frJar.toURI().toURL()},
            ResourceDecryptor3.class.getClassLoader()
        );

        // Load private key
        byte[] keyBytes = javax.xml.bind.DatatypeConverter.parseBase64Binary(PRIVATE_KEY_B64);
        PKCS8EncodedKeySpec spec = new PKCS8EncodedKeySpec(keyBytes);
        KeyFactory kf = KeyFactory.getInstance("RSA");
        PrivateKey privateKey = kf.generatePrivate(spec);

        // Get EncryptionToolBox.RSA class and decrypt method via reflection
        Class<?> rsaClass = frCL.loadClass("com.fr.security.encryption.core.EncryptionToolBox$RSA");
        Method decryptMethod = rsaClass.getMethod("decrypt", String.class, Key.class);
        System.out.println("Loaded: " + rsaClass.getName());

        Path inputPath = Paths.get(resourcesDir);
        Path outPath = Paths.get(outputDir);
        Files.createDirectories(outPath);

        int ok = 0, fail = 0, skip = 0;
        List<Path> files = new ArrayList<Path>();
        addFiles(inputPath, files);

        for (Path file : files) {
            String name = file.getFileName().toString().toLowerCase();
            if (!name.endsWith(".txt") && !name.endsWith(".md")) {
                skip++; continue;
            }
            Path rel = inputPath.relativize(file);
            Path outFile = outPath.resolve(rel);
            try {
                // Read file exactly as plugin does: BufferedReader line by line
                StringBuilder sb = new StringBuilder();
                BufferedReader br = Files.newBufferedReader(file, java.nio.charset.StandardCharsets.UTF_8);
                String line;
                while ((line = br.readLine()) != null) sb.append(line);
                br.close();

                String content = sb.toString().trim();
                if (content.isEmpty()) { skip++; continue; }

                // Decrypt using FR's EncryptionToolBox.RSA.decrypt()
                String decrypted = (String) decryptMethod.invoke(null, content, privateKey);

                Files.createDirectories(outFile.getParent());
                Files.write(outFile, decrypted.getBytes(java.nio.charset.StandardCharsets.UTF_8));
                System.out.println("  [OK] " + rel + " -> " + decrypted.length() + " chars");
                ok++;
            } catch (Exception e) {
                System.out.println("  [FAIL] " + rel + ": " + e.getMessage());
                fail++;
            }
        }
        frCL.close();

        System.out.println("\n=== Summary ===");
        System.out.println("Decrypted: " + ok);
        System.out.println("Failed:    " + fail);
        System.out.println("Skipped:   " + skip);
    }

    static void addFiles(Path dir, List<Path> result) {
        File d = dir.toFile();
        File[] children = d.listFiles();
        if (children == null) return;
        for (File child : children) {
            if (child.isDirectory()) addFiles(child.toPath(), result);
            else result.add(child.toPath());
        }
    }
}
